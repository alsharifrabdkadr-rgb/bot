...
# --- CONFIGURATION ---
BOT_TOKEN = os.getenv("8626665643:AAEA70xUxw-7mbabFEsB34xtUqYVfiTOsGs")
...
class Database:
    ...
    def get_leaderboard(self):
        with self.get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT telegram_id, teo_balance FROM users ORDER BY teo_balance DESC LIMIT 10")
            return cursor.fetchall()

    def get_all_users(self):
        with self.get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT telegram_id, teo_balance FROM users ORDER BY teo_balance DESC")
            return cursor.fetchall()
...
def format_curr(value):
    """Formats currency with 2 decimal places and commas."""
    return "{:,.2f}".format(value)

def get_user_display(user_id):
    """Gets username or first name if username is missing."""
    user = db.get_user(user_id)
    if not user: return "Unknown"
    # user[2] is username
    if user[2] and user[2] != "None":
        return "@" + user[2]
...
def ensure_user(update: Update):
    ...
    # Use actual username or first_name as fallback
    display_name = user.username if user.username else user.first_name
    
    if not db_user:
        db.create_user(user.id, display_name)
        return db.get_user(user.id)
    else:
        # Update username if it changed or was None
        if db_user[2] != display_name:
            db.update_user(user.id, username=display_name)
            return db.get_user(user.id)
    return db_user
...
def get_user_rank(user_id):
    lb = db.get_all_users()
    lb_ids = [row[0] for row in lb]
    try:
        rank = lb_ids.index(user_id) + 1
        return rank
    except ValueError:
        return "N/A"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if can_claim:
         ...
         await update.message.reply_text(STRINGS[lang]["daily_login_reward"].format(amount=format_curr(DAILY_LOGIN_REWARD)))
...
async def handle_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == STRINGS[lang]["btn_balance"]:
        ...
        await update.message.reply_text(STRINGS[lang]["balance_text"].format(
            ...
            teo=format_curr(user[3]), 
            euo=format_curr(user[4]), 
            level=user[12], 
            xp=int(user[11]), 
            rank=rank, 
            vip=vip_status, 
        ...
        for p, ts in history:
            ...
            table += f"{time_str}    {format_curr(p)} EUO\n"
            
        market_text = (
            f"📈 TEO Market\n\n"
            f"**Current Rate:**\n"
            f"1 TEO = {format_curr(m[0])} EUO\n\n"
            f"--- \n\n"
            f"{table}\n"
            f"--- \n\n"
            f"**Price Trend**\n\n"
        ...
        await update.message.reply_text(STRINGS[lang]["bank_menu"].format(bank_teo=format_curr(user[5]), bank_euo=format_curr(user[6])), reply_markup=InlineKeyboardMarkup(kb))
        ...
        count = 1
        for u_id, t in lb:
            display = get_user_display(u_id)
            txt += f"{count}. {display} — {format_curr(t)} TEO\n"
            count += 1
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("l_"):
        ...
        if last and now < datetime.fromisoformat(last) + timedelta(hours=24):
            ...
            await query.message.reply_text(STRINGS[lang]["daily_success"].format(amount=format_curr(DAILY_REWARD_AMOUNT)))
        ...
        if win > 0:
            ...
            await query.message.reply_text(STRINGS[lang]["wheel_win"].format(amount=format_curr(win)))
        ...
        if guess == correct:
            ...
            if user[20] and datetime.now() < datetime.fromisoformat(user[20]):
                win *= 2
            db.update_user(user_id, euo_balance=user[4] + win)
            await query.message.edit_text(STRINGS[lang]["guess_win"].format(amount=format_curr(win)))
        ...
        if res_val < 0.50: # 50% nothing
            ...
            result = STRINGS[lang]["box_small"].format(amount=format_curr(amt))
        elif res_val < 0.95: # 15% medium EUO
            amt = random.randint(300, 1000)
            if user[20] and datetime.now() < datetime.fromisoformat(user[20]): amt *= 2
            db.update_user(user_id, euo_balance=user[4] + amt)
            result = STRINGS[lang]["box_large"].format(amount=format_curr(amt))
        else: # 5% big (TEO)
            amt = random.randint(1, 3)
            db.update_user(user_id, teo_balance=user[3] + amt)
            result = STRINGS[lang]["box_rare"].format(amount=format_curr(amt))
        ...
        if user[20] and datetime.now() < datetime.fromisoformat(user[20]):
            win *= 2
        db.update_user(user_id, euo_balance=user[4] + win)
        await query.message.edit_text(STRINGS[lang]["quiz_win"].format(amount=format_curr(win)))
        await add_xp(user_id, 10, context)
    elif data == "q_lose":
        ...
        await query.message.reply_text(STRINGS[lang]["market_buy_ask"].format(price=format_curr(db.get_market()[0])))
        return MARKET_BUY_AMT
    elif data == "m_sell":
        await query.message.reply_text(STRINGS[lang]["market_sell_ask"].format(price=format_curr(db.get_market()[0])))
...
async def send_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user[bal_idx] >= amount:
        ...
        await update.message.reply_text(STRINGS[lang]["send_success"].format(amount=format_curr(amount), currency=currency, username=target[2]))
        await add_xp(user_id, 5, context)
    else:
...
async def convert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "teo_euo":
        if user[3] >= amount:
            euo_gain = amount * market_rate
            db.update_user(user_id, teo_balance=user[3] - amount, euo_balance=user[4] + euo_gain)
            await update.message.reply_text(STRINGS[lang]["convert_success"].format(euo=format_curr(euo_gain), teo=format_curr(amount)))
            await add_xp(user_id, 5, context)
        else:
            await update.message.reply_text(STRINGS[lang]["insufficient_balance"])
    elif mode == "euo_teo":
        ...
        if user[4] >= amount:
            teo_gain = amount / market_rate
            db.update_user(user_id, euo_balance=user[4] - amount, teo_balance=user[3] + teo_gain)
            await update.message.reply_text(STRINGS[lang]["convert_success"].format(euo=format_curr(amount), teo=format_curr(teo_gain)))
            await add_xp(user_id, 5, context)
        else:
            await update.message.reply_text(STRINGS[lang]["insufficient_balance"])
    else:
...
async def conv_convert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        mode = context.user_data.get('conv_mode', 'euo_teo')
        ...
        if mode == 'euo_teo':
            ...
            if amt > 0 and user[4] >= amt:
                ...
                await update.message.reply_text(STRINGS[lang]["convert_success"].format(euo=format_curr(amt), teo=format_curr(teo)))
            else: await update.message.reply_text(STRINGS[lang]["insufficient_balance"])
        else: # teo_euo
            if amt > 0 and user[3] >= amt:
                euo = amt * market_rate
                db.update_user(user[1], teo_balance=user[3] - amt, euo_balance=user[4] + euo)
                await update.message.reply_text(STRINGS[lang]["convert_success"].format(euo=format_curr(euo), teo=format_curr(amt)))
            else: await update.message.reply_text(STRINGS[lang]["insufficient_balance"])
    except: pass
    return MAIN_MENU_STATE
    ...
    try:
        ...
        if is_buy:
            if user[4] >= cost:
                ...
                new_price = price * (1 + (amt / 10000)) # Reduced impact for realism
...
async def update_market_periodically(context: ContextTypes.DEFAULT_TYPE):
    """Job to update market price with random volatility."""
    m = db.get_market()
    price = m[0]
    # Random change between -2% and +2%
    change = random.uniform(-0.02, 0.02)
    new_price = max(0.1, price * (1 + change))
    db.update_market(new_price, volume_inc=0)

def main():
    app = Application.builder().token(BOT_TOKEN).build()
    
    # Add job queue for market updates
    job_queue = app.job_queue
    job_queue.run_repeating(update_market_periodically, interval=300, first=10) # Every 5 minutes
